  ...
  return (
    <div className="space-y-8 animate-fade-up">
      <header className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
        <div>
           <h2 className="text-2xl sm:text-3xl font-display font-semibold text-slate-800 tracking-tight">Produtos & Estoque</h2>
          ...
          return (
            <div key={product.id} className="card-premium p-8 relative flex flex-col justify-between min-h-[300px] group transition-all">
               <div>
                  ...
                  <h3 className="text-xl font-semibold text-slate-800 uppercase leading-tight mb-1 tracking-tight">{product.name}</h3>
               ...
               <div className="mt-8 space-y-6">
                  <div className="flex items-center gap-12">
                     <div>
                        <p className="text-[9px] font-semibold text-slate-300 uppercase tracking-widest mb-1">Preço Venda</p>
                        <p className="text-xl font-semibold text-slate-800 font-mono tracking-tighter italic">{formatCurrency(product.salePrice)}</p>
                     </div>
                     <div>
                        <p className="text-[9px] font-semibold text-slate-300 uppercase tracking-widest mb-1">Disponível</p>
                        <p className="text-xl font-semibold text-slate-800 font-mono tracking-tighter">{product.stock} un</p>
                     </div>
                  </div>

                  <div className="pt-4 border-t border-slate-50 flex items-center justify-between">
                     <div className="flex items-center gap-1 text-green-500 font-semibold text-[10px] italic">
               ...
               {isAdmin && (
                 <div className="absolute top-6 right-6 flex gap-2">
                    <button onClick={() => { setEditingProduct(product); setFormData({ ...product }); setIsModalOpen(true); }} className="p-2.5 bg-white text-slate-400 hover:text-[#E38EA0] rounded-xl border border-slate-100 shadow-sm active:scale-90 transition-all"><Edit3 className="w-4 h-4" /></button>
          ...
          <div className="flex min-h-full items-start md:items-center justify-center">
            <div className="bg-white rounded-5xl w-full max-w-lg p-8 md:p-12 shadow-2xl animate-fade-up border border-pink-50 relative flex flex-col z-10 transition-all sm:my-auto">
              <div className="flex justify-between items-center mb-6 shrink-0 pr-8">
                <div>
                   <h3 className="text-xl md:text-2xl font-semibold text-slate-800 uppercase tracking-tight leading-none">{editingProduct ? 'Editar' : 'Novo'} Produto</h3>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <button type="submit" className="w-full btn-primary h-14 rounded-2xl text-[11px] uppercase tracking-widest font-semibold group shadow-xl transition-all active:scale-95 hover:shadow-pink-100 flex items-center justify-center gap-3">
