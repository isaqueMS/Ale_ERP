  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const data = {
        ...
        creatorName: profile?.name || currentUser?.displayName || 'Admin Studio',
      ...
      setFormData({ type: 'income', category: '', amount: 0, date: format(new Date(), 'yyyy-MM-dd'), description: '', professionalId: '', paymentMethod: '' });
    } catch (error) {
  ...
  return (
    <div className="space-y-10 pb-20 animate-fade-up">
      <header className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
        <div>
           <div className="flex items-center gap-2 mb-2">
             <div className="w-1.5 h-6 bg-[#E38EA0] rounded-full shadow-[0_0_10px_rgba(255,182,193,0.5)]" />
             <h2 className="text-2xl sm:text-3xl font-display font-semibold text-slate-800 tracking-tight">Contabilidade Geral</h2>
           </div>
      ...
      <div className="flex flex-col lg:flex-row gap-8">
         <div className="flex-1 bg-white rounded-5xl p-8 lg:p-10 shadow-sm border border-pink-50 space-y-8">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
               <div className="flex items-center gap-2 text-slate-800 font-semibold uppercase text-xs tracking-widest">
                 <Filter className="w-4 h-4 text-[#E38EA0]" /> Central de Inteligência
               </div>
               <div className="flex items-center gap-2 bg-[#FBF7F6] px-5 py-3 rounded-2xl border border-pink-50 shadow-sm">
                  <Calendar className="w-4 h-4 text-amber-500" />
                  <input type="date" value={startDate} onChange={e => setStartDate(e.target.value)} className="bg-transparent text-xs font-semibold text-slate-600 outline-none" />
                  <span className="text-xs font-bold text-slate-300">até</span>
                  <input type="date" value={endDate} onChange={e => setEndDate(e.target.value)} className="bg-transparent text-xs font-semibold text-slate-600 outline-none" />
               </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
               <div className="relative group">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 border-2 border-transparent rounded-3xl py-4 pl-14 pr-4 text-xs font-bold outline-none focus:bg-white focus:border-pink-50 transition-all placeholder:text-slate-300"
                    value={searchTerm}
                    onChange={e => setSearchTerm(e.target.value)}
                  />
               </div>
               <div className="relative group">
                  <User className="absolute left-5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-300 group-focus-within:text-[#E38EA0] transition-colors" />
                  <select
            ...
            <div className="flex flex-wrap gap-3 justify-center">
               {['all', 'income', 'expense'].map((t) => (
                 <button
                  ...
                  className={cn(
                    "px-10 py-3 rounded-2xl text-xs font-semibold uppercase tracking-widest transition-all",
                    filterType === t
                      ? "bg-[#E38EA0] text-white shadow-lg shadow-pink-100"
      ...
      <section className="space-y-6">
        <div className="flex items-center gap-2 ml-4">
           ...
           <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-widest">Divisão de Ganhos</h3>
        </div>
           ...
           {staffCommissions.map(s => (
             <div key={s.id} className="card-premium p-10 relative overflow-hidden group">
                ...
                <div className="relative h-full flex flex-col justify-between">
                   <div className="mb-10">
                      <div className="flex items-center gap-2 mb-2">
                        <span className="text-[10px] font-semibold text-[#E38EA0] uppercase tracking-widest">Profissional</span>
                        <div className="h-0.5 flex-1 bg-slate-50" />
                      </div>
                      <h4 className="text-xl font-semibold text-slate-800 uppercase tracking-tighter leading-none mb-1">{s.name}</h4>
                      <p className="text-[10px] font-bold text-slate-300 uppercase tracking-widest">{s.count} ATENDIMENTOS CONCLUÍDOS</p>
                   </div>

                   <div className="space-y-5 mb-10">
                      <div className="flex items-center justify-between">
                         <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest">Faturamento</span>
                         <span className="text-sm font-semibold text-emerald-500 font-mono">{formatCurrency(s.totalRevenue)}</span>
                      </div>
                      <div className="flex items-center justify-between">
                         <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest">Comissão ({s.percentage}%)</span>
                         <span className="text-sm font-semibold text-amber-500 font-mono">{formatCurrency(s.totalCommission)}</span>
                      </div>
                   </div>

                   <div className="pt-6 border-t border-slate-50">
                      <div className="flex items-center justify-between">
                         <span className="text-[10px] font-semibold text-slate-800 uppercase tracking-widest">Lucro Líquido</span>
                         <span className="text-2xl font-semibold text-slate-800 font-mono tracking-tighter">{formatCurrency(s.netProfit)}</span>
                      </div>
                   </div>
                </div>
             </div>
      ...
      <div className="card-premium p-0 overflow-hidden">
        <div className="p-10 border-b border-slate-50 bg-[#FBF7F6] flex flex-col md:flex-row justify-between items-center gap-4">
           <h3 className="text-xs font-semibold text-slate-800 uppercase tracking-widest flex items-center gap-3">
             <FileText className="w-5 h-5 text-[#E38EA0]" /> Histórico de Transações
           </h3>
           ...
           <table className="w-full min-w-[1000px]">
              <thead>
                 <tr className="text-[10px] font-semibold text-slate-300 uppercase tracking-widest bg-slate-50/30">
                   ...
                   <th className="px-10 py-6 text-left">Pagamento</th>
              ...
              <tbody className="divide-y divide-slate-50">
                 {filteredTransactions.map(t => {
                   ...
                   const paymentLabel = PAYMENT_METHODS.find(pm => pm.value === t.paymentMethod)?.label;
                   return (
                    <tr key={t.id} className="hover:bg-pink-50/20 transition-all group">
                       <td className="px-10 py-6">
                          <p className="text-xs font-semibold text-slate-800">{format(parseISO(t.date), 'dd/MM/yyyy')}</p>
                       ...
                       <td className="px-10 py-6">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                               <p className="text-xs font-semibold text-slate-700 uppercase tracking-tight">{t.category}</p>
                               <p className="text-[10px] font-bold text-slate-400 mt-1">{t.description || 'Consulta Avulsa'}</p>
                            </div>
                          </div>
                       </td>
                       <td className="px-10 py-6">
                          <span className="bg-[#FBF7F6] border border-pink-50 text-slate-600 text-[10px] font-semibold px-3 py-1.5 rounded-xl">@{professional?.name?.split(' ')[0] || 'Studio'}</span>
                       </td>
                       <td className="px-10 py-6">
                          {paymentLabel ? (
                            <span className="bg-slate-50 border border-slate-100 text-slate-500 text-[10px] font-semibold px-3 py-1.5 rounded-xl">{paymentLabel}</span>
                          ) : (
                            <span className="text-[10px] font-semibold text-slate-200">—</span>
                          )}
                       </td>
                       <td className="px-10 py-6 text-right font-mono font-semibold text-base">
          ...
          <div className="flex min-h-full items-start md:items-center justify-center">
            <div className="bg-white rounded-5xl w-full max-w-lg p-8 md:p-12 shadow-2xl animate-fade-up border border-pink-50 relative flex flex-col z-10 transition-all sm:my-auto">
              <div className="flex justify-between items-center mb-6 shrink-0 pr-8">
                <div>
                  <h3 className="text-xl md:text-2xl font-semibold text-slate-800 uppercase tracking-tight leading-none">{editingTransaction ? 'Ajustar' : 'Novo'} Lançamento</h3>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="flex gap-2 p-2 bg-slate-50 rounded-2xl shadow-inner mb-2">
                   <button type="button" onClick={() => setFormData({...formData, type: 'income'})} className={cn("flex-1 py-3 rounded-xl text-[10px] font-semibold uppercase tracking-widest transition-all", formData.type === 'income' ? "bg-white text-emerald-500 shadow-md scale-[1.02]" : "text-slate-400 opacity-60 hover:opacity-100")}>Entrada</button>
                   <button type="button" onClick={() => setFormData({...formData, type: 'expense'})} className={cn("flex-1 py-3 rounded-xl text-[10px] font-semibold uppercase tracking-widest transition-all", formData.type === 'expense' ? "bg-white text-red-500 shadow-md scale-[1.02]" : "text-slate-400 opacity-60 hover:opacity-100")}>Saída</button>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                   ...
                   <div className="relative pt-1.5 font-sans">
                      ...
                      <input required type="date" className="input-premium !py-3 !px-5 font-semibold uppercase text-[10px] shadow-sm" value={formData.date} onChange={e => setFormData({...formData, date: e.target.value})} />
                   </div>
                </div>

                <div className="relative pt-1.5 font-sans">
                   <label className="floating-label">Forma de Pagamento</label>
                   <select className="select-premium !py-3 !px-5 text-[10px] uppercase tracking-widest" value={formData.paymentMethod || ''} onChange={e => setFormData({...formData, paymentMethod: e.target.value})}>
                ...
                <button type="submit" className="w-full btn-primary h-14 rounded-2xl text-[11px] uppercase tracking-widest font-semibold group shadow-xl active:scale-95 transition-all flex items-center justify-center gap-3">
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 bg-slate-900/10 backdrop-blur-md z-[100] flex items-center justify-center p-4">
          <div className="bg-white p-10 rounded-5xl shadow-2xl max-w-sm w-full text-center space-y-8 animate-fade-up border border-pink-50">
            ...
            <h3 className="text-xl font-semibold text-slate-800 uppercase tracking-tight">Remover Lançamento?</h3>
            <div className="flex gap-4">
              <button onClick={() => setDeleteConfirm(null)} className="flex-1 py-4 rounded-2xl font-semibold text-slate-400 uppercase text-xs tracking-widest hover:bg-slate-50 transition-all">Cancelar</button>
              <button onClick={async () => { await deleteDoc(doc(db, 'transactions', deleteConfirm)); setDeleteConfirm(null); }} className="flex-1 py-4 rounded-2xl bg-red-500 text-white font-semibold uppercase text-xs tracking-widest shadow-xl hover:bg-red-600 transition-all">Confirmar</button>
            </div>
          </div>
        </div>
      )}
...
function SummaryPillar({ title, value, color, icon: Icon, isMain }: any) {
  return (
    <div className={cn(
      "p-8 rounded-5xl border transition-all flex flex-col justify-between h-36 relative overflow-hidden group hover:scale-[1.03]",
      isMain ? "bg-slate-800 border-transparent text-white shadow-2xl shadow-slate-200" : "bg-white border-pink-50 text-slate-800 shadow-sm"
       ...
       {isMain && <div className="absolute top-0 right-0 w-32 h-32 bg-[#E38EA0] rounded-full -mr-16 -mt-16 blur-2xl opacity-20" />}
       <p className={cn("text-[10px] font-semibold uppercase tracking-widest", isMain ? "text-pink-200" : "text-slate-400")}>{title}</p>
       <div className="flex items-end justify-between">
          <p className={cn("text-3xl font-semibold font-mono tracking-tighter", isMain ? "text-white" : color)}>
            {formatCurrency(value)}
          </p>
          <Icon className={cn("w-6 h-6", isMain ? "text-[#E38EA0]" : "text-slate-200")} />
       </div>
    </div>
  );
}
