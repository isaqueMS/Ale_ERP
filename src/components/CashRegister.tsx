...
export default function CashRegister() {
  ...
  if (!isAdmin) {
    return <div className="p-20 text-center font-bold text-slate-400 uppercase tracking-widest animate-fade-in">Acesso Restrito ao Painel de Administração</div>;
  }

  return <CashRegisterAdmin />;
}

function CashRegisterAdmin() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const data = {
      ...
      creatorId: user?.uid,
      creatorName: profile?.name || user?.displayName || 'Studio Alexandra',
    ...
    setFormData({ category: '', amount: '', description: '', date: format(new Date(), 'yyyy-MM-dd'), paymentMethod: '' });
  };
  ...
  return (
    <div className="space-y-10 animate-fade-up pb-20">
      <header className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-6">
        <div>
           <h2 className="text-3xl sm:text-4xl font-display font-semibold text-slate-800 tracking-tight">Fluxo de Caixa</h2>
        ...
        <div className="flex gap-4 w-full sm:w-auto">
           <button onClick={() => { setModalType('income'); setIsModalOpen(true); }} className="flex-1 sm:flex-none h-14 sm:h-16 px-6 sm:px-8 rounded-2xl bg-emerald-500 text-white font-semibold uppercase text-[10px] sm:text-xs tracking-widest shadow-lg shadow-emerald-100 flex items-center justify-center gap-2 transition-all hover:scale-[1.02] active:scale-95">
              <Plus className="w-5 h-5" /> Receita
           </button>
           <button onClick={() => { setModalType('expense'); setIsModalOpen(true); }} className="flex-1 sm:flex-none h-14 sm:h-16 px-6 sm:px-8 rounded-2xl bg-red-400 text-white font-semibold uppercase text-[10px] sm:text-xs tracking-widest shadow-lg shadow-red-100 flex items-center justify-center gap-2 transition-all hover:scale-[1.02] active:scale-95">
      ...
      <div className="space-y-6">
        <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
           <div className="relative w-full md:w-96 group">
              <Search className="absolute left-6 top-1/2 -translate-y-1/2 w-4 h-4 text-[#E38EA0] transition-transform group-focus-within:scale-110" />
              <input
                type="text"
           ...
           <button className="flex items-center gap-2 text-[10px] font-semibold text-slate-400 uppercase tracking-widest bg-white px-8 py-5 rounded-2xl border border-slate-100 shadow-sm hover:bg-[#E38EA0] hover:text-white transition-all active:scale-95 group">
           ...
           <div className="divide-y divide-slate-50">
                ...
                <div key={t.id} className="p-8 flex flex-col md:flex-row md:items-center justify-between gap-6 transition-all hover:bg-[#FBF7F6] group relative">
                   <div className="flex items-center gap-6">
                      ...
                      <div>
                         <div className="flex items-center gap-2 mb-1">
                            <span className="text-[10px] font-semibold text-slate-300 uppercase tracking-widest">{format(parseISO(t.date), "dd MMM, yyyy", { locale: ptBR })}</span>
                            {isToday(parseISO(t.date)) && <span className="text-[8px] font-semibold text-emerald-500 uppercase bg-emerald-50 px-2.5 py-1 rounded-full animate-pulse">Hoje</span>}
                            {paymentLabel && <span className="text-[8px] font-semibold text-slate-400 uppercase bg-slate-50 border border-slate-100 px-2.5 py-1 rounded-full">{paymentLabel}</span>}
                         </div>
                         <h4 className="text-lg font-semibold text-white mix-blend-difference uppercase leading-none mb-1 tracking-tight">{t.category}</h4>
                   ...
                   <div className="flex items-center gap-8 justify-between md:justify-end">
                      <div className="text-right">
                         <p className={cn(
                           "text-2xl font-semibold font-mono tracking-tighter leading-none italic",
                         ...
                         <div className="flex items-center justify-end gap-1.5 mt-1.5 opacity-60">
                            ...
                            <p className="text-[8px] font-semibold text-slate-300 uppercase tracking-widest">Registrado por {t.creatorName || 'Sistema'}</p>
                         </div>
                      </div>
              ...
              {filteredTransactions.length === 0 && (
                <div className="p-20 text-center">
                   <div className="w-20 h-20 bg-slate-50 rounded-4xl flex items-center justify-center mx-auto mb-6">
                      <FileText className="w-8 h-8 text-slate-200" />
                   </div>
                   <h3 className="text-xl font-semibold text-slate-300 uppercase tracking-tighter italic">Nenhum lançamento encontrado</h3>
          ...
          <div className="flex min-h-full items-start md:items-center justify-center">
            <div className="bg-white rounded-5xl w-full max-w-lg p-8 md:p-12 shadow-2xl animate-fade-up border border-pink-50 relative flex flex-col z-10 transition-all sm:my-auto">
              <div className="flex justify-between items-center mb-6 shrink-0 pr-8">
                <div>
                  <h3 className="text-xl md:text-2xl font-semibold text-slate-800 uppercase tracking-tight leading-none">{editingTransaction ? 'Editar' : 'Nova'} {modalType === 'income' ? 'Receita' : 'Despesa'}</h3>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  ...
                  <div className="relative pt-1.5 font-sans">
                    ...
                    <input required type="date" className="input-premium !py-3 !px-5 shadow-sm font-semibold text-[10px] uppercase" value={formData.date} onChange={(e) => setFormData({...formData, date: e.target.value})} />
                  </div>
                </div>
                {modalType === 'income' && (
                  <div className="relative pt-1.5 font-sans">
                ...
                <button type="submit" className={cn(
                  "w-full h-14 rounded-2xl text-[11px] font-semibold uppercase tracking-widest transition-all flex items-center justify-center gap-3 shadow-xl active:scale-95 group",
...
function BalanceCard({ title, value, icon: Icon, color, isMain }: any) {
  return (
    <div className={cn(
      "p-8 rounded-5xl relative overflow-hidden transition-all group hover:scale-[1.02]",
      isMain ? "bg-[#E38EA0] text-white shadow-2xl shadow-pink-100" : "bg-white text-slate-800 shadow-sm border border-pink-50"
    )}>
       <div className={cn(
         "w-12 h-12 rounded-2xl flex items-center justify-center mb-6",
         isMain ? "bg-white/20 text-white" : "bg-pink-50 text-[#E38EA0]"
       )}>
          <Icon className="w-6 h-6" />
       </div>
       <p className={cn("text-[10px] font-semibold uppercase tracking-widest mb-2", isMain ? "text-white/80" : "text-slate-400")}>{title}</p>
       <p className="text-3xl font-semibold font-mono tracking-tighter leading-none">{formatCurrency(value)}</p>
    </div>
  );
}
